...
resource "aws_subnet" "app_servers_subnet" {
  ...
  cidr_block              = cidrsubnet(data.terraform_remote_state.vpc.outputs.vpc_cidr_block,8 ,var.instance_netnum)
  map_public_ip_on_launch = local.map_public_ip
  availability_zone       = var.availability_zone
  tags = {
    Name ="${var.cluster_name}_app_servers_subnet_${var.instance_netnum}"
  }
} # end resource
